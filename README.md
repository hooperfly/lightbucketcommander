...
## Version:
This is beta version 0.3.3 of the Light Bucket Commander(LBC) and subject to major refactoring.
...
On startup, the LBC reads the main Paparazzi UAV configuration file `$PPRZ_HOME/conf/conf.xml`. 
Configuration data contained in the `conf.xml` and the associated airframe/flight plan files is 
used to initialize the LBC server. Note that the `--subscribe` option is also used to initialize 
runtime related aircraft data that includes `ivy bus messages`. It is recommended to start the LBC server
prior to starting the Paparazzi server to acquire a complete message dictionary. The `--file` option 
is used to configure client related data( see `lbc_conf.xml` for an example of the default client configuration file).
...
The generate switch (`-g`, `--generate`) parses the Paparazzi `conf.xml` file and generates a LBC compliant xml
...
- [klingon_conf.xml](https://github.com/paparazzi/lightbucketcommander/blob/master/klingon_conf.xml)
- [klingon icons folder](https://github.com/paparazzi/lightbucketcommander/tree/master/static/images/klingon)
...
Each row of aircraft buttons will use the respective color assigned to each aircraft in the `lbc_conf.xml` file, 
a row major view.
...
a browser(e.g. Chrome, Firefox, etc...) to execute the respective client view. 
    ...
    localhost:5000/show/status/   (NOTE: Inovke the LBC server with the -s/--subscribe option)
...
be used to populate this view. It is recommended to start the LBC server prior to starting the Paparazzi 
server to acquire a complete message dictionary.

    python lbc.py -i 192.168.1.147 -s
![Alt Status View](doc/images/status_screen.png?raw=true "Status View")

## Video Demos:
Here are a couple of informal demo videos of the Light Bucket Commander; captured from
Periscope broadcasts:

- [LBC: Flight Block Video](https://www.youtube.com/watch?v=NgT0K1RzfmE)
- [LBC: Guidance Video](https://www.youtube.com/watch?v=BdItVWyjLUc)
...
- [x] Added support in `lbc_conf.xml` for the following aircraft attributes: `color`, `label`, `icon` and `tooltip` 
...
- [x] Added client configuration by name="string" in `lbc_conf.xml` (i.e. name="stay_p1")
...
- [x] Refactor lbc.py with direct calls to the IvyMessagesInterface
